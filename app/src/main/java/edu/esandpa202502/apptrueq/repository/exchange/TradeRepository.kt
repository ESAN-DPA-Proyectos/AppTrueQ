...
class TradeRepository {
    ...
    suspend fun acceptOffer(offer: Offer): Result<Unit> = try {
        db.runTransaction {
            ...
            val notificationToOfferer = NotificationItem(
                userId = offer.ownerId,
                title = "¡Tu oferta fue aceptada!",
                message = "El dueño de '${offer.needText}' aceptó tu oferta por '${offer.title}'.",
                type = "offer_accepted"
            )
            val notificationToNeedOwner = NotificationItem(
                userId = offer.needOwnerId,
                title = "Oferta Aceptada",
                message = "Has aceptado la oferta de ${offer.ownerName} para tu necesidad '${offer.needText}'.",
                type = "offer_accepted"
            )
    ...
    suspend fun rejectOffer(offer: Offer): Result<Unit> = try {
        db.runBatch {
            ...
            val notification = NotificationItem(
                ...
                message = "Tu oferta por '${offer.needText}' fue rechazada.",
                type = "offer_rejected"
            )
    ...
    private fun getOffersByField(fieldName: String, userId: String): Flow<List<Offer>> =
        callbackFlow {
            val listener = db.collectionGroup("offers")
